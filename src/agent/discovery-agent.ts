  ...
  async discover(request: DiscoveryRequest): Promise<{ trace: RunTrace; result: RunResult }> {
    ...
    try {
      ...
      for (let index = 0; index < request.maxSteps; index++) {
        ...
        await this.evidence.append({ timestamp: new Date().toISOString(), runId, mode: "discovery", event: "action", stepId, payload: { action: action.action, targetId: "target" in action ? action.target?.id : undefined, policy: authorization.reason, success: actionResult.success, resolution: actionResult.resolution, duration_ms: actionResult.durationMs, output: action.action === "extract" ? action.output : undefined, message: actionResult.message } });
